			...
			while(true) {
				...
				while(true){
					...
					try{
						...
						if(((getRequest(receivedPacket.getData()) == Request.DATA) && (getPacketNum(receivedPacket) < blockNumber))){
							byte dupAckData[] = new byte[]{0, 4, receivedPacket.getData()[2], receivedPacket.getData()[3]};
							...
							continue;
						} else if ((getRequest(receivedPacket.getData()) == Request.DATA) && ((rollOver) && (blockNumber != getPacketNum(receivedPacket)))){
							byte dupAckData[] = new byte[]{0, 4, receivedPacket.getData()[2], receivedPacket.getData()[3]};
							sendPacket.setData(dupAckData);
							socket.sendPacket(sendPacket);
							if(isVerbose()){
				...
				if ((getPacketNum(receivedPacket) > blockNumber) && (!rollOver)) {
					error_code = 4;
			...
			while ((n = in.read(fileData,0,512)) != -1) {
				...
				while(true){
					...
					try{
						...
						if(((getRequest(receivedPacket.getData()) == Request.ACK) && (getPacketNum(receivedPacket) < blockNumber)) || ((rollOver) && (blockNumber != getPacketNum(receivedPacket)))){
							if(isVerbose()){
				...
				if (isErrorPacket(receivedPacket)) {
					...
					if (code == 4) { /* Signals that the the transfer should end. */
						...
						return;
					} else { // default behavior for errors
						return;
					}
				}
				...
				while(true){
					...
					try{
						...
						if(((getRequest(receivedPacket.getData()) == Request.ACK) && (getPacketNum(receivedPacket) < blockNumber))){
							...
							continue;
						} else if((getRequest(receivedPacket.getData()) == Request.ACK) && ((rollOver) && (blockNumber != getPacketNum(receivedPacket)))){
							if(isVerbose()){
								System.out.println("\n Duplicate ACK Packet Received.\n");
							}
							continue;
