...
public class TFTPClient {
	...
	private static final int SIM_RECV_PORT = 23;
	private static final int SERVER_RECV_PORT = 69;
	...
	private boolean rollOver = false; // Set when packets numbers roll over 65535
			...
			while(true){
				...
				while(true){
					...
					try{
						...
						if((getPacketNum(receivePacket) < blockNumber) && (receivePacket.getData()[1] == 3)){
							byte dupAckData[] = new byte[]{0, 4, receivePacket.getData()[2], receivePacket.getData()[3]};
							sendPacket.setData(dupAckData);
							sendPacketToHost(sendReceiveSocket, sendPacket);
							if(isVerbose()){
							...
							continue;
						} else if (((rollOver) && (blockNumber != getPacketNum(receivePacket))) && (receivePacket.getData()[1] == 3)){
							byte dupAckData[] = new byte[]{0, 4, receivePacket.getData()[2], receivePacket.getData()[3]};
							sendPacket.setData(dupAckData);
							sendPacketToHost(sendReceiveSocket, sendPacket);
							if(isVerbose()){
		...
		try {
			...
			while ((n = in.read(fileData,0,512)) != -1){
				...
				while(true){
					...
					try{
						sendReceiveSocket.receive(receivePacket);
						if((receivePacket.getData()[1] == 4) && (getPacketNum(receivePacket) < blockNumber)){
							if(isVerbose()){
								System.out.println("\n Duplicate ACK Packet Received.\n");
							}
							continue;
						} else if(((rollOver) && (blockNumber != getPacketNum(receivePacket))) && (receivePacket.getData()[1] == 4)){
							if(isVerbose()){
								System.out.println("\n Duplicate ACK Packet Received.\n");
							}
							continue;
	...
	private void printPacketData(boolean send, DatagramPacket packet){
		try{
			...
			System.out.println("\nCan't print data, array out of bounds. ");
		}
	}
			...
			while(true){ // Until the IP Address is valid, keeps requesting valid path
				...
				if(userInput(response.toUpperCase())){
					...
					try { // Initialize server address to local host
						...
						System.out.println("Could not set localHost. Maybe try again?");
						continue;
					}
					return true;
				} try {
					...
					System.out.println("Error Code 5: Could not set address. Choose a different address:");
					continue;
				}
			}	
	...
	public void start(){
		...
		while(true){ // Loops until user quits
			...
			while(true){ // Until the user response valid, keep asking
				System.out.println("Would you like to transfer another file? (Y/N) ('SA' to change server IP or H for help)");
				String response = responseScanner.nextLine().toUpperCase();
				if(response.equals("Y")){
					break; // break out of loop
				} else if(response.equals("N")){
